#!/bin/bash
# Persistent LaTeX Setup for Emergent
# Installs LaTeX to /app/.texlive which persists across restarts

TEXLIVE_DIR="/app/.texlive"
...
apt-get install -y --no-install-recommends fonts-inter > /dev/null 2>&1
